...
2. Fisher Information实际上就是score function的二阶矩:$I(\theta)=E[S(Y^{n},\theta)^{2}]$。
3. 我们发现：$Var(S(Y^{n},\theta))=E(S(Y^{n},\theta)^{2})-[E(S(Y^{n}),\theta)]^{2}=E(S(Y^{n},\theta)^{2})=I(\theta)$因此我们得到了Fisher information的第一个数学意义：*用来估计MLE方程的方差*。它的直观表示就是，随着收集的数据越来越多，这个方差由于是一个independent sum的形式，也就变得越来越大也就象征着得到的信息越来越多。
